...
int main(void)
{
    ...
    for (int i = 0, sentenceLen = strlen(text); i < sentenceLen; i++) //loop for text
    {
        ...
        else if (index == 33 || index == 46 || index == 63) // count sentences
        {
            sentenceCount++;
        }
        lastIndex = index;
    ...
    grade = 0.0588 * (100 * (float)letterCount / (float)wordCount) - 0.296 *
            (100 * (float)sentenceCount / (float)wordCount) - 15.8; // Coleman-Liau index
